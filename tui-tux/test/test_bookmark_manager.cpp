...
TEST_F(BookmarkTest, BookmarkCommand_InvalidHistoryIndex) {
    ...
    testing::internal::CaptureStdout();
    // act
    mock_bookmark_manager.bookmark(99, "alias1", session_history);
    // assert
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_EQ(output, "Error: Invalid history index.\n");
    EXPECT_TRUE(mock_bookmark_manager.bookmarks.empty());
}
...
TEST_F(BookmarkTest, BookmarkCommand_ExecutesQueryIfResultNotFound) {
    ...
    EXPECT_CALL(mock_agency_manager, execute_query("query1", ::testing::_))
        .WillOnce(::testing::Return("executed_result"));
    // act
    mock_bookmark_manager.bookmark(1, "alias2", session_history);
    // assert
    ASSERT_EQ(mock_bookmark_manager.bookmarks.size(), 1);
    EXPECT_EQ(mock_bookmark_manager.bookmarks["alias2"].first, "query1");
